...
- **`@opentui-ui/{core,react,solid}/dialog`** — the foundation compound
  primitive.
- **`registry/dialog`** — a validated preview recipe, not part of the
  six-family starter catalog.
- **`@opentui-ui/dialog`** — the adopted companion package with manager,
...
```bash React
pnpm dlx shadcn@4.13.0 add @opentui-ui/react/dialog
```

```bash Solid
pnpm dlx shadcn@4.13.0 add @opentui-ui/solid/dialog
```

```bash Core
pnpm dlx shadcn@4.13.0 add @opentui-ui/core/dialog
```
