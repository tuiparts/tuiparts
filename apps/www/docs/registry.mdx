...
The OpenTUI UI registry distributes editable recipe source through the
...
```json title="components.json"
{
  ...
  "registries": {
    "@opentui-ui": "https://opentui-ui.dev/r/{name}.json"
  }
}
```

Then install items by address:
...
```bash
pnpm dlx shadcn@4.13.0 add https://opentui-ui.dev/r/react/checkbox.json
```
...
`@opentui-ui/toast` packages remain the production companion paths.
...
```bash
pnpm dlx shadcn@4.13.0 add @opentui-ui/react/checkbox --view
pnpm dlx shadcn@4.13.0 add @opentui-ui/react/checkbox --diff
pnpm dlx shadcn@4.13.0 add @opentui-ui/react/checkbox --dry-run
```
